#include <stdlib.h>
#include <string.h>

#include "grid.h"
#include "math_utils.h"

Grid2D *grid_create(int width, int height) {
  Grid2D *grid = (Grid2D *)malloc(sizeof(Grid2D));
  if (!grid)
    return NULL;

  grid->data = (float *)malloc(width * height * sizeof(float));
  if (!grid->data) {
    free(grid);
    return NULL;
  }

  grid->width = width;
  grid->height = height;

  memset(grid->data, 0, width * height * sizeof(float));
  return grid;
}

void grid_destroy(Grid2D *grid) {
  if (grid) {
    free(grid->data);
    free(grid);
  }
}

void grid_clear(Grid2D *grid) {
  if (grid) {
    memset(grid->data, 0, grid->width * grid->height * sizeof(float));
  }
}

void grid_copy(Grid2D *dest, Grid2D *src) {
  if (dest->height == src->height && dest->width == src->width) {
    memcpy(dest->data, src->data, src->width * src->height * sizeof(float));
  }
}

float grid_get(Grid2D *grid, int x, int y) {
  if (!grid)
    return 0.0f;

  x = (x < 0) ? 0 : (x >= grid->width) ? grid->width - 1 : x;
  y = (y < 0) ? 0 : (y >= grid->height) ? grid->height - 1 : y;

  return grid->data[y * grid->width + x];
}

float grid_get_physics(Grid2D *grid, int x, int y, int boundary_type) {
  if (!grid)
    return 0.0f;

  if (x >= 0 && x < grid->width && y >= 0 && y < grid->height) {
    return grid->data[y * grid->width + x];
  }

  if (x < 0 || x >= grid->width) {
    int safe_x = (x < 0) ? 0 : grid->width - 1;
    float val = grid_get(grid, safe_x, y);
    return (boundary_type == 1) ? -val : val;
  }

  if (y < 0 || y >= grid->height) {
    int safe_y = (y < 0) ? 0 : grid->height - 1;
    float val = grid_get(grid, x, safe_y);
    return (boundary_type == 2) ? -val : val;
  }

  return 0.0f;
}

void grid_set(Grid2D *grid, int x, int y, float value) {
  if (grid && x >= 0 && x < grid->width && y >= 0 && y < grid->height) {
    grid->data[y * grid->width + x] = value;
  }
}

void grid_add_source(Grid2D *grid, int x, int y, float amount) {
  if (grid && x >= 0 && x < grid->width && y >= 0 && y < grid->height) {
    grid->data[y * grid->width + x] += amount;
  }
}

float grid_interpolate(Grid2D *grid, float x, float y) {
  if (grid && x >= 0 && x < grid->width && y >= 0 && y < grid->height) {
    int x0 = (int)x;
    int x1 = x0 + 1;
    int y0 = (int)y;
    int y1 = y0 + 1;

    float fx = x - x0;
    float fy = y - y0;

    float tl = grid_get(grid, x0, y0);
    float tr = grid_get(grid, x1, y0);
    float bl = grid_get(grid, x0, y1);
    float br = grid_get(grid, x1, y1);

    return bilinear_interpolate(tl, tr, bl, br, fx, fy);
  }

  return 0.0f;
}

float laplacian(Grid2D *grid, int x, int y) {
  if (!grid)
    return 0.0f;

  int left_x = (x > 0) ? x - 1 : x;
  int right_x = (x < grid->width - 1) ? x + 1 : x;
  int up_y = (y > 0) ? y - 1 : y;
  int down_y = (y < grid->height - 1) ? y + 1 : y;

  float center = grid_get(grid, x, y);
  float left = grid_get(grid, left_x, y);
  float right = grid_get(grid, right_x, y);
  float up = grid_get(grid, x, up_y);
  float down = grid_get(grid, x, down_y);

  return (left + right + up + down) - 4.0f * center;
}

float laplacian_physics(Grid2D *grid, int x, int y, int boundary_type) {
  if (!grid)
    return 0.0f;

  float left = grid_get_physics(grid, x - 1, y, boundary_type);
  float right = grid_get_physics(grid, x + 1, y, boundary_type);
  float up = grid_get_physics(grid, x, y - 1, boundary_type);
  float down = grid_get_physics(grid, x, y + 1, boundary_type);
  float center = grid_get(grid, x, y);

  return (left + right + up + down) - 4.0f * center;
}
